...
pub fn upload_strings(strings: Vec<String>) -> Result<(), String> {
    let mut data = STRINGS.lock().map_err(|_| "Failed to acquire lock".to_string())?;
...
pub fn k_nearest_neighbour_sort(query: String, k: u32, word_order_sensitive: Option<bool>) -> Result<Vec<String>, String> {
    let word_order_sensitive = word_order_sensitive.unwrap_or(true);
    let data = STRINGS.lock().map_err(|_| "Failed to acquire lock".to_string())?;
    ...
    let mut distances: Vec<(usize, &StringData)> = data.iter()
        .map(|item| {
...
mod tests {
    ...
    fn test_upload_and_search() {
        ...
        let sensitive_results = k_nearest_neighbour_sort("aple".to_string(), 2, Some(true)).unwrap();
        assert_eq!(sensitive_results.len(), 2);
        assert_eq!(sensitive_results[0], "apple");
        
        // Test word-order independent search
        let word_independent_results = k_nearest_neighbour_sort(
            "premium device techno".to_string(),
            2,
            Some(false)
        ).unwrap();
